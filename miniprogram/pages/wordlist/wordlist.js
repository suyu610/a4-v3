...
Page({
  ...
  selectRightAll() {
    ...
    let wordRightCheckedList = []
    if (!allSelectMode) {
      wordList.forEach(e => {
        wordRightCheckedList.push(e.wordName)
      })
    }
    this.setData({
      wordRightCheckedList,
  ...
  onRightBtnTapped(e) {
    ...
    let wordRightCheckedList = this.data.wordRightCheckedList
    // 存在则删除
    let index = wordRightCheckedList.indexOf(word)
    if (index != -1) {
      wordRightCheckedList.splice(index, 1)
    } else {
      // 不存在
      wordRightCheckedList.push(word)
    }
    this.setData({
      wordRightCheckedList
    })
  ...
  selectAll() {
    ...
    // 如果相等，则执行清空操作，否则全部置入
    if (wordLeftCheckedList.length != wordList.length) {
      wordList.forEach(e => {
        tmp.push(e.wordName)
      })
    }
